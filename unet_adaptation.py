#!/usr/bin/env python3
"""
Domain Adaptation with Maximum Mean Discrepancy (MMD) for Semantic Segmentation using U-Net
...
def train_and_validate_epoch(
    ...
    for i in tqdm(range(steps), desc="Training"):
        ...
        source_encoder_out = model.encoder(source_data)
        source_embeddings = source_encoder_out[-1]

        target_encoder_out = model.encoder(target_data)
        target_embeddings = target_encoder_out[-1]
...
def create_model(num_classes=19, device=None, pretrained_path=None):
    """Create and load the U-Net model"""
    ...
    if pretrained_path and os.path.exists(pretrained_path):
        ...
        # Filter out aux_classifier keys if they exist
