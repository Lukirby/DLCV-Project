...
def train_and_validate_epoch_multilevel_mmd(
            ...
            aspp_mmd_weight=0.5  # Fraction of total MMD weight for ASPP (0.0 to 1.0)
    ):
    if device is None:
    ...
    for i in tqdm(range(steps), desc="Training"):
        ...
        # Extract ASPP features
        source_aspp_features = model.classifier[0](source_embeddings)  # ASPP output
        target_aspp_features = model.classifier[0](target_embeddings)
        ...
        mmd_loss = (1-aspp_mmd_weight) * backbone_mmd_loss + aspp_mmd_weight * aspp_mmd_loss

        loss = cls_loss + mmd_loss * mmd_weight
        ...
        del source_aspp_features, target_aspp_features
        del loss, cls_loss, mmd_loss, backbone_mmd_loss, aspp_mmd_loss
...
def domain_adaptation_multilevel(
        start_epoch, end_epoch,
        model, name,
        optimizer, criterion, 
        syn_train_dataloader, syn_val_dataloader, 
        real_train_dataloader, real_val_dataloader,
        seed=42, num_classes=19, mmd_weight=0.01, lr=1e-4
    ):
    ...
    for epoch in range(resume_epoch, end_epoch):
        ...
        source_train_avg_loss, source_train_avg_iou, target_train_avg_iou, \
            source_val_avg_loss, source_val_avg_iou, target_val_avg_iou = \
        train_and_validate_epoch_multilevel_mmd(
            model, optimizer, criterion, 
            syn_train_dataloader, syn_val_dataloader,
            real_train_dataloader, real_val_dataloader,
